...
//프로젝트 상세
   ...
   String sql = "SELECT pj_name,pj_img, pj_start,pj_end,pj_team, ct_seq, pj_ref, pj_text, pj_hit FROM project WHERE pj_seq=" + tmp; // 
   ...
   sql = "SELECT us_id FROM project a JOIN project_member b ON a.pj_seq = b.pj_seq WHERE a.pj_seq=" + tmp + " AND b.us_id = '" + id + "'";
   rs = null;
   rs = stmt.executeQuery(sql);
...
%>
  ...
  <div>
      <table align="center" width="98%">
          <tr>
              <td colspan="2"><b><u> <%= pj[0] %> </u><img src="project_logo.png" width="30" height="30"/>조회수 <%=pj[8] %></b><br></td>
              <td align="right"><% if(is_member.equals("true")){out.print("<form action='project_modification.jsp'><input type='hidden' value='"+id+"'><input type='submit' value='수정'></form>");} %></td>
          </tr>
          <tr>
...
%>                    
  ...
  <div>
      <table  width="98%" align="center">
          <tr>
              <th align="right"><a href ="project_data_modification.jsp?seq=<%= tmp %>&id=<%= id%>"><% if(is_member.equals("true")){out.print("<input type='button' value='수정'>");}%></a></th>
          </tr>
      </table>
...
//프로젝트 커뮤니티
   tmp = (String)request.getParameter("seq");
   sql="SELECT us_img, pc_text, a.us_id, pc_seq FROM project_community a INNER JOIN user b ON a.us_id = b.us_id WHERE pj_seq=" + tmp;
   stmt.executeQuery(sql);
   ...
   if(rs.next()){
      do{
         ...
         out.print("</tr>");
         if(id.equals(rs.getString("a.us_id"))){
            out.print("<tr>");
            out.print("<td></td>");
            out.print("<td align='right'>");
                ...
                out.print("<input type='hidden' name='seq' value='" + tmp + "'>");
...
%>
        ...
        <!-- 커뮤니티 글 입력하기 -->
        <div>
            <form action="executeUpdate.jsp" target='hidden'>
                &nbsp;&nbsp;
                <input type="hidden" name='id' value=<%= id %>>
...
<%
   ...
   sql = "update project set pj_hit= pj_hit+1 where pj_seq=" + tmp;
