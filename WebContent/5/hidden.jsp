...
<%
	...
	if( request.getParameter("comment") != null){
		...
		if(request.getParameter("comment_update") != null){
			//sql = "DELETE FROM user WHERE us_id = '" + sess + "';";
			sql = "UPDATE project_community SET pc_text = '" + request.getParameter("comment_update") + "' WHERE pc_seq = "+ request.getParameter("comment") + ";";
			out.print("코멘트 수정");
			out.print("sql: "+sql);
			//out.print("결과: " + stmt.executeUpdate(sql));
			//session.invalidate();
...
%>
<%
	if(request.getParameter("check") != null){out.print(request.getParameter("check"));}
	if(request.getParameter("sql") != null){out.print(request.getParameter("sql"));}
	//resultset 해제
